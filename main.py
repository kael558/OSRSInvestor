...
def buy(max, min, current):
    threshhold = (max-min)/5 + min
...
for itemID in itemList:
    ...
    max = max(current_daily_dict)
    min = min(current_daily_dict)
    current = current_daily_dict[-1]

    shouldBuy = buy(max, min, current)
    ...
    r2 = requests.get('http://services.runescape.com/m=itemdb_oldschool/api/catalogue/detail.json',
                      params={'item': itemID})
